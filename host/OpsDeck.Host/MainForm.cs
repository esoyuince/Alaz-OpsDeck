...
public sealed class MainForm : Form
{
    ...
    {
        this.settings=settings;Text="ALAZ OPSDECK — M6.15-A / Responsiveness";ClientSize=new Size(1280,780);MinimumSize=new Size(1040,660);StartPosition=FormStartPosition.CenterScreen;
        ...
        timer.Tick+=(_,_)=>RefreshStatus();Shown+=(_,_)=>{Text="ALAZ OPSDECK — M6.15-A / Responsiveness";engine=new(settings.Load(),settings){Locked=sessionLocked};engine.Start();engine.StartCodexTelemetry();timer.Start();if(startInTray)BeginInvoke(new Action(()=>{ShowInTaskbar=false;Hide();}));};

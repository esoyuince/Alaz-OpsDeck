...
public sealed class MainForm : Form
{
    ...
    {
        this.settings=settings;Text="ALAZ OPSDECK — M6.20-A / Codex Quota Resilience";ClientSize=new Size(1280,780);MinimumSize=new Size(1040,660);StartPosition=FormStartPosition.CenterScreen;
        ...
        timer.Tick+=(_,_)=>RefreshStatus();Shown+=(_,_)=>{Text="ALAZ OPSDECK — M6.20-A / Codex Quota Resilience";engine=new(settings.Load(),settings){Locked=sessionLocked};engine.Start();engine.StartCodexTelemetry();timer.Start();if(startInTray)BeginInvoke(new Action(()=>{ShowInTaskbar=false;Hide();}));};
    ...
    private void RefreshStatus()
    {
        ...
        var rdc=engine.DesktopCommander;string rdcValue=sessionLocked?"Kilitli":rdc.ProcessCount>0?$"LOCAL ACTIVE · {rdc.ProcessCount} proc":"OFFLINE";string rdcDetail=sessionLocked?"Windows oturumu kilitli; RDC ayrıntısı gizlendi.":rdc.RunCalls.HasValue?$"Bu remote run: {rdc.RunCalls:N0} tool call · {rdc.RunSessions??0} session. Lifetime: {rdc.TotalCalls??0:N0} call · {rdc.Sessions??0} session · başarı {(rdc.SuccessPermille.HasValue?rdc.SuccessPermille.Value/10d:0):0.0}% · son tool {(string.IsNullOrWhiteSpace(rdc.LastTool)?"—":rdc.LastTool)}":rdc.TotalCalls.HasValue?$"Lifetime {rdc.TotalCalls:N0} tool call · {rdc.Sessions??0} session · son tool {(string.IsNullOrWhiteSpace(rdc.LastTool)?"—":rdc.LastTool)}":"Remote process gözlemi var; yerel usageStats bulunamadı.";Row("Remote Desktop Commander",rdc.State.ToString(),rdcValue,rdcDetail);

...
{
    public string Wire()=>JsonSerializer.Serialize(new {type="opsdeck.pc.v1",cpu=Cpu,cpu_temp=CpuTemp,cpu_sensor=CpuTemp.HasValue?1:4,chassis_temp=ChassisTemp,chassis_sensor=ChassisTemp.HasValue?1:4,fan_sensor=Fan1Rpm.HasValue||Fan2Rpm.HasValue?1:4,volumes=(Volumes??[]).Take(DiskTelemetry.PanelLimit).Select(v=>v.Wire(DateTimeOffset.UtcNow)),volume_count=(Volumes??[]).Length,gpu=Gpu,gpu_temp=GpuTemp,intel_gpu=IntelGpu,intel_temp=IntelTemp,intel_shared_gib=IntelSharedGib,intel_shared_limit_gib=IntelSharedLimitGib,fan1_rpm=Fan1Rpm,fan2_rpm=Fan2Rpm,ram_used_gib=RamUsedGib,ram_total_gib=RamTotalGib,vram_used_gib=VramUsedGib,vram_total_gib=VramTotalGib,rx_mbps=RxMbps,tx_mbps=TxMbps},Json.Options);
}
...
public sealed record FleetState(AgentSample Agents,Metric Workers,Metric D1,Metric R2,Metric Hosting,Metric Cost)
{
    ...
    public string Wire(DateTimeOffset now,bool locked,LinkHealthSnapshot? link=null,CodexUsageSnapshot? codexUsage=null,ManagedCodexSnapshot? managedCodex=null)
    {
        ...
        return JsonSerializer.Serialize(new {type="opsdeck.status.v1",locked,agents=new{
            ...
            rdc_process_count=locked?-1:Agents.RemoteDesktop?.ProcessCount??-1,rdc_total_calls=locked?-1:Counter(Agents.RemoteDesktop?.TotalCalls),
            rdc_sessions=locked?-1:Counter(Agents.RemoteDesktop?.Sessions),rdc_success_permille=locked?-1:Agents.RemoteDesktop?.SuccessPermille??-1,

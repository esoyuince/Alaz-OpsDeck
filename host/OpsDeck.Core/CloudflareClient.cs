...
public sealed partial class CloudflareClient : IDisposable
{
    ...
    public static Metric ParseCost(JsonElement root,string account)
    {
        ...
        if(rows.GetArrayLength()==0)return new(SourceState.NoData,CollectedAt:DateTimeOffset.UtcNow,Detail:"No billing rows; not a zero-cost claim");
        DateTimeOffset? periodStart=null,periodEnd=null;bool firstPeriod=true,periodMismatch=false;decimal total=0;string? currency=null;DateTimeOffset? sourceEnd=null;HashSet<string> keys=[];
        foreach(var row in rows.EnumerateArray()){
            ...
            if(currency!=null&&currency!=cur)throw new SourceFailure(SourceState.Partial,"Multiple currencies; no combined total");currency=cur;
            string Field(string name)=>row.TryGetProperty(name,out var x)?x.ToString():"";
            DateTimeOffset? ps=DateTimeOffset.TryParse(Field("BillingPeriodStart"),CultureInfo.InvariantCulture,DateTimeStyles.AssumeUniversal,out var pStart)?pStart:null;
            DateTimeOffset? pe=DateTimeOffset.TryParse(Field("BillingPeriodEnd"),CultureInfo.InvariantCulture,DateTimeStyles.AssumeUniversal,out var pEnd)?pEnd:null;
            if(ps.HasValue&&pe.HasValue&&pe<=ps)throw new SourceFailure(SourceState.Error,"Invalid billing period order");
            if(firstPeriod){periodStart=ps;periodEnd=pe;firstPeriod=false;}else if(periodStart!=ps||periodEnd!=pe)periodMismatch=true;
        ...
        if(periodMismatch)return new(SourceState.Partial,CollectedAt:DateTimeOffset.UtcNow,Detail:"Mixed billing periods; no total");
        var metric=new Metric(SourceState.Ok,(double)total,null,DateTimeOffset.UtcNow,sourceEnd,currency!,
            ...
            periodStart,periodEnd,Note:"USAGE RECORDS",UsageCharges:true);
        return metric with{State=Freshness.MetricState(metric,DateTimeOffset.UtcNow,7200)};
    }

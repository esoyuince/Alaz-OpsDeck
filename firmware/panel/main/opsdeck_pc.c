...
bool opsdeck_pc_decode(const cJSON *o,opsdeck_pc_t *s){
 ...
 if(s->fan_sensor!=1&&(s->valid&(PC_FAN1|PC_FAN2)))return false;
 const char *fan_control_names[]={"fan_control_supported","fan_manual_supported","fan_control_mode","fan_control_speed_pct","fan_control_busy"};int fan_control_present=0;
 for(int i=0;i<5;i++)if(cJSON_HasObjectItem(o,fan_control_names[i]))fan_control_present++;
 if(fan_control_present!=0&&fan_control_present!=5)return false;
 if(fan_control_present==5){
  if(!integer(o,"fan_control_supported",0,1,&s->fan_control_supported)||!integer(o,"fan_manual_supported",0,1,&s->fan_manual_supported)||!integer(o,"fan_control_mode",0,3,&s->fan_control_mode)||!integer(o,"fan_control_speed_pct",0,100,&s->fan_control_speed_pct)||!integer(o,"fan_control_busy",0,1,&s->fan_control_busy))return false;
  if(!s->fan_control_supported&&(s->fan_manual_supported||s->fan_control_mode!=0||s->fan_control_speed_pct!=0||s->fan_control_busy))return false;
  if(s->fan_manual_supported&&(s->fan_control_speed_pct<50||s->fan_control_speed_pct>100||s->fan_control_speed_pct%5!=0))return false;
  if(s->fan_control_mode==3&&!s->fan_manual_supported)return false;
